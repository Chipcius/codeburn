...
describe('rememberGeneration', () => {
  it('serves every period from one generation', () => {
    rememberGeneration(payload(totals()), 1_000)
    expect(generationHeadline('today')).toEqual({ cost: 700, calls: 6000 })
    expect(generationHeadline('lifetime')).toEqual({ cost: 37800, calls: 272000 })
    expect(generationAt()).toBe(1_000)
  })

  it('keeps the newest generation and ignores an older payload arriving late', () => {
    rememberGeneration(payload(totals()), 2_000)
    rememberGeneration(payload(totals({ today: [1, 1] })), 1_000)
    expect(generationHeadline('today')).toEqual({ cost: 700, calls: 6000 })
  ...
  it('holds the last good generation when a payload carries no totals', () => {
    ...
    expect(generationHeadline('all')).toEqual({ cost: 37700, calls: 270000 })
    expect(generationAt()).toBe(1_000)
  })
