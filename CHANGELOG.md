...
### Added
- **The desktop app's Plugins page has a Menu bar card for the macOS menubar app.** On macOS it shows one of three states: nothing installed and an Install button; installed, with its version and Open and Uninstall; running, with a green dot, the version, Open, Quit, Uninstall and a Capacity Dock switch. Install runs the same `codeburn menubar` the command line does, so the release is pinned to the desktop's own version, the checksum and the bundle identity are verified before anything is moved, an older copy is quit before it is replaced and exactly one ends up running. A menubar installed by hand or by the CLI is found in `~/Applications`, `/Applications`, or wherever Spotlight knows its bundle identifier, so the card describes what is on the machine rather than only what it put there. Quit and Uninstall ask the menubar to go rather than killing it — the login item is the app's own to drop, which nothing outside it can do — and fall back to a signal for a build too old to be asked; Uninstall then takes the bundle away from wherever it was found and the card returns to Not installed. Both confirm in the card itself rather than in a system dialog, the way removing a plugin already does on this page. The Capacity Dock switch writes the menubar's own preference, which a running menubar now acts on within a second. There is no link step — both apps already share the CLI, the cache and the config — and no login-item prompt, because the menubar registers itself on first launch. Failures read as sentences (offline, not published for this version, damaged download, no permission), never a stack trace. Install is behind a capability flag that a Mac App Store build turns off, where the card points at the website instead; nothing renders on Windows or Linux.
