...
### Added
- **Every `codeburn serve --stdio` response carries the generation it was built from.** Each answer the resident process derives advances a counter, and the response reports it alongside the day range that answer covers (`{"generation":{"n":4,"from":"2026-09-09","to":"2026-09-16"}}`); an answer served from the memo carries the stamp of the derivation it came from. A client holding several panels can tell which of them share one reading of the corpus and show a single clock for it instead of one per panel.
...
### Fixed
- **A period switch in the desktop app no longer re-reads every session transcript in the window.** The optimize detectors run their own scan of every Claude transcript overlapping the period, separate from the parse, and it was re-read in full on every request - about six of the seven seconds a warm 30-day switch cost. A resident `codeburn serve --stdio` process now keeps each scanned file, keyed by the file's path, size and last write plus the range it was scanned for, so only files written since the last request are read again; the recency flag every detector reads is stamped after the scan rather than during it, so a kept file is not pinned to the cutoff it was first read under, and the whole scan now shares one cutoff instead of taking a fresh clock reading per file. The memo is bounded at 192MB with age and least-recently-used eviction, and is dropped with the other in-memory memos when the process passes its resident-set guard. On a 7GB corpus, with a session written between every request, a warm `status --format menubar-json --period 30days` goes from 7759ms to 2152ms, `--period all` from 8963ms to 2764ms and `--period week` from 2991ms to 1355ms; with nothing written between requests all three are answered from the memo in about 1ms. No cache format changed, so no cache version was bumped, and every payload is byte-identical to before.
- **The resident `codeburn serve --stdio` process stopped re-deriving every period on every request.** The desktop app's server keeps a memo per query and reuses it while no watched session root has changed, but SQLite rewrites a database's shared-memory index when it is *read*, so opening a provider database under a watched root reported a change — CodeBurn invalidating, on every request, the memo it had just produced. `-shm` writes are now ignored (a real write still lands in the database or its WAL, both watched), so a repeat query on an unchanged corpus is answered from the memo instead of re-deriving the whole period. Two further changes cut what a period switch costs when the corpus *has* changed: a change is now scoped to the days the changed file itself can hold — the days between its creation and its last write — so a finalized past range keeps its result when only today's sessions were written; and loaded cache shards stay parsed in the process, keyed by the shard file name (which carries a fresh nonce on every write, so a name that is still published names the same bytes), bounded to 192MB and evicted by age and least-recent use, so widening the range from 30 days to six months reads only the months it has not read yet. On a 7GB corpus a warm `report --format json --period 30days` goes from 2294ms to 1337ms and `--period all` from 3139ms to 1907ms, and a repeat of an unchanged query from seconds to about 1ms. No cache format changed, so no cache version was bumped; every payload is byte-identical to before.
